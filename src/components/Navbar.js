// ============================================================
// Navbar.js — Sidebar (desktop) + Top Bar (mobile), v2 — 11 menu
// ============================================================
...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { id: 'kurs',       label: 'Kurs & Rate',          icon: '📈' },
      { id: 'transfer',   label: 'Transfer Domestik',    icon: '🔀' },
      { id: 'remittance', label: 'Transfer Luar Negeri', icon: '🌐' },
    ],
  },
  {
    label: 'Informasi',
    items: [
      { id: 'berita',     label: 'Berita & Pengumuman',  icon: '📢' },
      { id: 'likuiditas', label: 'Likuiditas Cabang',    icon: '🏦' },
...
export default function Navbar({ currentPage, setCurrentPage }) {
  ...
  const activeItem = ALL_ITEMS.find((n) => n.id === currentPage);
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="ml-auto flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-2 bg-mint-50 text-mint-600 px-3 py-1.5 rounded-full text-xs font-semibold">
                <span className="w-2 h-2 bg-mint-500 rounded-full animate-pulse inline-block" /> Sistem Aktif
              </div>
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-sm font-bold">T</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* SIDEBAR */}
      <aside className={`fixed top-0 left-0 h-full z-50 bg-white border-r border-gray-100 flex flex-col transition-transform duration-300 w-[260px]
        ...
        <nav className="flex-1 px-4 py-3 space-y-4 overflow-y-auto">
          {NAV_GROUPS.map((group) => (
            <div key={group.label}>
              <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider px-3 mb-1">{group.label}</div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-200
                        ${active ? 'bg-primary-500 text-white shadow-md shadow-primary-200' : 'text-gray-600 hover:bg-gray-50 hover:text-primary-600'}`}
                    >
        ...
        <div className="p-4 border-t border-gray-100">
          <div className="bg-gradient-to-r from-primary-500 to-primary-600 rounded-2xl p-3 text-white">
            ...
            <div className="text-xs opacity-90 leading-relaxed">Selalu verifikasi keaslian uang asing dengan metode 3D: Dilihat, Diraba, Diterawang.</div>
          </div>
          <button onClick={() => app?.resetAllData?.()} className="mt-3 w-full text-center text-xs text-gray-400 hover:text-red-500 transition-colors">
            Reset data ke default
          </button>
          <div className="mt-1 text-center text-xs text-gray-300">CEOD v2.0 © 2026 Bank BTN</div>
        </div>
      </aside>
    </>
  );
